#!/bin/bash
# Ablation: w.o. SMG (no surrogate model generation, use PSL only)
...
for seed in $seeds; do for task in $tasks; do for train_mode in $train_modes; do
    ...
    CUDA_VISIBLE_DEVICES=$gpu python DOMOO/domoo.py --model=$model --train_mode=$train_mode --task=$task --retrain_model=False --seed=$seed --results_dir=./ablation/no_surrogate --no_surrogate=True &
done; done; done
wait && echo "ALL DONE"
